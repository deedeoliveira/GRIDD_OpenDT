...
test("load-public loads and activates exactly the four runtime artifacts", async () => {
    const { database, graph, loader } = realSetup();
    const results = await loader.loadPublic();

    assert.equal(results.length, 4);
    assert.equal(graph.putCalls.length, 4);
    assert.equal(database.families.filter((family) => family.current_artifact_id !== null).length, 4);
