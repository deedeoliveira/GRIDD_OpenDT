...
export class ApplicationIdentityDatabase {
  ...
  async listLocalAccounts(): Promise<ApplicationAccount[]> { await this.db.checkConnection(); const [rows]:any=await this.db.connection.execute("SELECT * FROM application_accounts WHERE account_key IN ('pg202404','TEST-ACTOR-REVOKED-001','TEST-ACCOUNT-DISABLED-001') ORDER BY account_key"); return rows; }
  async createSession(accountId:number, tokenHash:string, expiresAt:Date):Promise<{sessionUuid:string}> { await this.db.checkConnection(); const sessionUuid=crypto.randomUUID(); await this.db.connection.execute("INSERT INTO application_sessions (session_uuid,application_account_id,token_hash,status,expires_at,created_by_provider) VALUES (:sessionUuid,:accountId,:tokenHash,'active',:expiresAt,'local_synthetic_session')",{sessionUuid,accountId,tokenHash,expiresAt}); return {sessionUuid}; }
