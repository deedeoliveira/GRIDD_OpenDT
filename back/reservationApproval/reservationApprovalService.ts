...
export class ReservationApprovalService {
  ...
  async managerScopes(accountId:number, allowEmpty=false) { await this.db.checkConnection(); const [rows]:any=await this.db.connection.execute(`SELECT s.id,s.asset_id,s.scope_uuid FROM reservation_management_scopes s JOIN application_account_roles ar ON ar.application_account_id=s.application_account_id AND ar.revoked_at IS NULL JOIN application_roles r ON r.id=ar.application_role_id AND r.normalized_role_key='reservation_manager' JOIN application_accounts a ON a.id=s.application_account_id AND a.status='active' WHERE s.application_account_id=:accountId AND s.status='active'`,{accountId}); if(!rows.length&&!allowEmpty) throw new ReservationApprovalError('reservation_manager_scope_denied','This account has no active reservation-management scope.',403); return rows; }
  async list(accountId:number,input:{page?:unknown;pageSize?:unknown;status?:unknown}={}) {
    const scopes=await this.managerScopes(accountId,true); const ids=scopes.map((s:any)=>Number(s.asset_id));
    ...
    if(!ids.length) return {items:[],page,pageSize,totalItems:0,totalPages:1};
  ...
  async decide(accountId:number,sessionUuid:string,reservationId:number,kind:'approved'|'rejected'|'cancelled',input:{reason?:unknown;overrideAcknowledged?:unknown}) {
    const scopes=await this.managerScopes(accountId); const decisionReason=reason(input.reason,kind==='rejected'||kind==='cancelled');
